...
def test_create_workflow(db):
    ...
    created_workflow = WorkflowRoutes.create_workflow(data=workflow_data, db=db)
...
def test_update_workflow(db):
    ...
    workflow_to_update = WorkflowRoutes.create_workflow(data=WorkflowCreateSchema(name=workflow_name), db=db)

    updated_workflow = WorkflowRoutes.update_workflow(workflow_id=workflow_to_update.id, data=WorkflowCreateSchema(name=new_workflow_name), db=db)
...
def test_delete_workflow(db):
    workflow_name = "Test Workflow"
    workflow = WorkflowRoutes.create_workflow(data=WorkflowCreateSchema(name=workflow_name), db=db)
    deleted_workflow = WorkflowRoutes.delete_workflow(workflow_id=workflow.id, db=db)
...
def test_create_and_run_graph(db): # Valid data 
    ...
    start_node = NodeRoutes.create_start_node(node_data=start_node_data, db=db)
    message_node = NodeRoutes.create_message_node(node_data=message_node_data, db=db)
    condition_node = NodeRoutes.create_condition_node(node_data=condition_node_data, db=db)
    no_node = NodeRoutes.create_message_node(node_data=no_node_data, db=db)
    yes_node = NodeRoutes.create_message_node(node_data=yes_node_data, db=db)
    end_node = NodeRoutes.create_end_node(node_data=end_node_data, db=db)
    ...
    NodeRoutes.update_start_node(node_id=start_node.id, node_data=start_node_data, db=db)

    message_node_data = MessageNodeSchema(workflow_id=workflow.id, message_text='test', status='sent', next_node_id=condition_node.id)
    NodeRoutes.update_message_node(node_data=message_node_data, node_id=message_node.id, db=db)
    ...
    NodeRoutes.update_condition_node(node_data=condition_node_data, node_id=condition_node.id, db=db)
    ...
    NodeRoutes.update_message_node(node_data=yes_node_new_data , node_id=yes_node.id, db=db)
    ...
    NodeRoutes.update_message_node(node_data=no_node_new_data , node_id=no_node.id, db=db)

    # Running sequence
    result = WorkflowRoutes.run_sequence(workflow_id=workflow.id, db=db)
